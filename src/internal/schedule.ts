...
/** @internal */
export const duration = (duration: Duration.Duration): Schedule.Schedule<never, unknown, Duration.Duration> =>
...
export const exponential = (
  ...
  delayedSchedule(map(forever, (i) => Duration.millis(base.millis * Math.pow(factor, i))))

/** @internal */
export const fibonacci = (one: Duration.Duration): Schedule.Schedule<never, unknown, Duration.Duration> =>
  delayedSchedule(
    pipe(
      unfold(
        [one, one] as const,
...
/** @internal */
export const fromDelay = (delay: Duration.Duration): Schedule.Schedule<never, unknown, Duration.Duration> =>
  duration(delay)
  ...
  makeWithState(
    ...
    (now, input, state) =>
      core.flatMap(self.step(now, input, state), ([state, out, decision]) => {
        ...
        return core.map(f(out, delay), (duration) => {
...
/** @internal */
export const recurUpTo = (duration: Duration.Duration): Schedule.Schedule<never, unknown, Duration.Duration> =>
