...
export const done = Debug.methodWithTrace((trace) =>
  <E, A>(exit: Exit.Exit<E, A>): Effect.Effect<never, E, A> => suspend(() => exit).traced(trace)
)
...
/* @internal */
export const if_ = Debug.dualWithTrace<
  <R1, R2, E1, E2, A, A1>(
    onTrue: Effect.Effect<R1, E1, A>,
    onFalse: Effect.Effect<R2, E2, A1>
...
/* @internal */
export const attemptOrElse = Debug.dualWithTrace<
  <R2, E2, A2, A, R3, E3, A3>(
    that: LazyArg<Effect.Effect<R2, E2, A2>>,
    onSuccess: (a: A) => Effect.Effect<R3, E3, A3>
...
/* @internal */
export const uninterruptible: <R, E, A>(self: Effect.Effect<R, E, A>) => Effect.Effect<R, E, A> = Debug.methodWithTrace(
  (trace) =>
    <R, E, A>(self: Effect.Effect<R, E, A>): Effect.Effect<R, E, A> => {
      const effect = new EffectPrimitive(OpCodes.OP_UPDATE_RUNTIME_FLAGS) as any
...
/** @internal */
export const currentTracerSpanAnnotations: FiberRef.FiberRef<HashMap.HashMap<string, string>> = globalValue(
