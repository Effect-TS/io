  ...
  describe.concurrent("Counter", () => {
    it.effect("custom increment as aspect", () =>
      Effect.gen(function*() {
        ...
        const result = yield* pipe(
          counter(Effect.unit()),
          Effect.zipRight(counter(Effect.unit())),
    ...
    it.effect("count + taggedWith", () =>
      Effect.gen(function*() {
        ...
        const counter = pipe(
          base,
          Metric.taggedWith((input: string) => HashSet.make(MetricLabel.make("dyn", input)))
        )
        const result = yield* pipe(
          Effect.succeed("hello"),
          Effect.withMetric(counter),
  ...
  describe.concurrent("Frequency", () => {
    ...
    it.effect("occurences + taggedWith", () =>
      Effect.gen(function*() {
        ...
        const frequency = pipe(
          base,
          Metric.taggedWith((s: string) => HashSet.make(MetricLabel.make("dyn", s)))
        )
    ...
    it.effect("custom observe as aspect", () =>
      Effect.gen(function*() {
        ...
        const histogram = pipe(
          Metric.histogram("h1", boundaries),
          Metric.taggedWithLabels(labels)
        )
        const result = yield* pipe(
          Effect.succeed(1),
