...
describe("Effect", () => {
  it.effect("sync forever is interruptible", () =>
    Effect.gen(function*($) {
      ...
      expect(Exit.isFailure(result) && Cause.isInterruptedOnly(result.i0)).toBeTrue()
    }))
  ...
  it.effect("running an effect preserves interruption status", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Deferred.await(deferred))
      const result = yield* $(Fiber.interrupt(fiber))
      expect(Exit.isFailure(result) && Exit.isInterrupted(result) && Cause.isInterruptedOnly(result.i0)).toBeTrue()
    }))
