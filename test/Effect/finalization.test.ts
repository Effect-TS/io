...
describe("Effect", () => {
  ...
  it.effect("finalizer errors reported", () =>
    Effect.gen(function*($) {
      ...
      expect(reported !== undefined && Exit.isSuccess(reported)).toBeFalse()
    }))
  ...
  it.live("acquireUseRelease regression 1", () =>
    Effect.gen(function*($) {
      ...
      expect(pipe(result, ReadonlyArray.findFirst((s) => s === "start 2"), Option.isSome)).toBeTrue()
      expect(pipe(result, ReadonlyArray.findFirst((s) => s === "release 2"), Option.isSome)).toBeTrue()
    }))
