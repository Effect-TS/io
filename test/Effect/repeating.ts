...
describe.concurrent("Effect", () => {
    ...
    Effect.gen(function*() {
      const effect = (ref: Ref.Ref<number>) => {
        return pipe(
          ...
          Effect.flatMap((n) =>
            n < 10 ?
              pipe(ref, Ref.update((n) => n + 1), Effect.zipRight(Effect.fail("Ouch"))) :
              Effect.succeed(n)
          )
        )
      }
  ...
  it.effect("repeatUntil - repeats until condition is true", () =>
    Effect.gen(function*() {
      ...
      yield* pipe(
        ...
        Effect.zipLeft(pipe(output, Ref.update((n) => n + 1))),
  ...
  it.effect("repeatUntil - always evaluates effect at least once", () =>
    Effect.gen(function*() {
      const ref = yield* Ref.make(0)
      yield* pipe(ref, Ref.update((n) => n + 1), Effect.repeatUntil(constTrue))
  ...
  it.effect("repeatUntilEquals - repeats until result is equal to predicate", () =>
    Effect.gen(function*() {
      ...
      yield* pipe(Queue.take(queue), Effect.zipLeft(pipe(ref, Ref.update((n) => n + 1))), Effect.repeatUntilEquals(5))
      const result = yield* Ref.get(ref)
  ...
  it.effect("repeatUntilEffect - always evaluates the effect at least once", () =>
    Effect.gen(function*() {
      const ref = yield* Ref.make(0)
      yield* pipe(ref, Ref.update((n) => n + 1), Effect.repeatUntilEffect(() => Effect.succeed(true)))
      const result = yield* Ref.get(ref)
      assert.strictEqual(result, 1)
    }))
  ...
  it.effect("repeatWhile - always evaluates the effect at least once", () =>
    Effect.gen(function*() {
      const ref = yield* Ref.make(0)
      yield* pipe(ref, Ref.update((n) => n + 1), Effect.repeatWhile(constFalse))
      const result = yield* Ref.get(ref)
      assert.strictEqual(result, 1)
    }))
  ...
  it.effect("repeatWhileEquals - repeats while the result equals the predicate", () =>
    Effect.gen(function*() {
      ...
      yield* pipe(Queue.take(queue), Effect.zipLeft(pipe(ref, Ref.update((n) => n + 1))), Effect.repeatWhileEquals(0))
      const result = yield* Ref.get(ref)
      assert.strictEqual(result, 5)
    }))
  ...
  it.effect("repeatWhileEffect - always evaluates effect at least once", () =>
    Effect.gen(function*() {
      const ref = yield* Ref.make(0)
      yield* pipe(ref, Ref.update((n) => n + 1), Effect.repeatWhileEffect(() => Effect.succeed(false)))
      const result = yield* Ref.get(ref)
      assert.strictEqual(result, 1)
    }))
})
