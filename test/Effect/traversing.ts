...
describe.concurrent("Effect", () => {
  ...
  it.effect("loop - loops with the specified effectual function", () =>
    Effect.gen(function*() {
      ...
      yield* Effect.loop(
        ...
        (n) => n < 5,
        (n) => n + 1,
        (n) => pipe(ref, Ref.update(List.prepend(n)))
      )
  ...
  it.effect("forEachDiscard - runs effects in order", () =>
    Effect.gen(function*() {
      ...
      yield* pipe(
        [1, 2, 3, 4, 5],
        Effect.forEachDiscard((n) => pipe(ref, Ref.update(List.prepend(n))))
      )
      const result = yield* pipe(Ref.get(ref), Effect.map(List.reverse))
  ...
  it.effect("forEachDiscard - can be run twice", () =>
    Effect.gen(function*() {
      ...
      const effect = pipe(
        [1, 2, 3, 4, 5],
        Effect.forEachDiscard((n) => pipe(ref, Ref.update((_) => _ + n)))
      )
      yield* effect
      yield* effect
  ...
  it.effect("forEachOption - succeeds with None given None", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        ...
        Effect.forEachOption((s) => Effect.succeed(s.length))
      )
  ...
  it.effect("forEachPar - runs single task", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        [2],
        Effect.forEachPar((n) => Effect.succeed(n * 2))
      )
  ...
  it.effect("forEachPar - returns results in the same order", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        ["1", "2", "3"],
        Effect.forEachPar((s) => Effect.sync(() => Number.parseInt(s)))
      )
      assert.deepStrictEqual(Array.from(result), [1, 2, 3])
    }))
  ...
  it.effect("forEachPar - parallelism - returns the results in the appropriate order", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        [1, 2, 3],
        Effect.forEachPar((n) => pipe(Effect.succeed(n.toString()))),
  ...
  it.effect("forEachPar - parallelism - works on large lists", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.forEachPar((n) => Effect.succeed(n)),
  ...
  it.effect("forEachParDiscard - accumulates errors", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        [1, 2, 3],
        Effect.forEachParDiscard((n) =>
          pipe(
            task(started, trigger, n),
  ...
  it.effect("forEachParDiscard - completes on empty input", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        [],
        Effect.forEachParDiscard(() => Effect.unit())
      )
      assert.isUndefined(result)
    }))
