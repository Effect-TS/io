...
describe.concurrent("Effect", () => {
  ...
  it.effect("acquireUseRelease - use is interruptible", () =>
    Effect.gen(function*($) {
      const fiber = yield* $(
        pipe(
          Effect.acquireUseRelease(Effect.unit, { use: () => Effect.never, release: () => Effect.unit }),
          Effect.fork
        )
      )
      const result = yield* $(Fiber.interrupt(fiber))
  ...
  it.effect("interrupted cause persists after catching", () =>
    Effect.gen(function*($) {
      ...
      assert.isTrue(pipe(
        result,
        Chunk.reduce(true, (acc, curr) => acc && Exit.isFailure(curr) && Cause.isInterruptedOnly(curr.i0))
      ))
    }))
