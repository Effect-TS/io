...
describe.concurrent("Effect", () => {
  ...
  it.effect("noneOrFail - on Some fails", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        Effect.noneOrFail(Option.some("some")),
  ...
  it.effect("noneOrFailWith - on Some fails", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        Effect.noneOrFailWith(Option.some("some"), (s) => s + s),
        Effect.catchAll(Effect.succeed)
      )
  ...
  it.effect("someOrElseEffect - extracts the value from Some", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        Effect.succeed(Option.some(1)),
        Effect.someOrElseEffect(() => Effect.succeed(42))
      )
      assert.strictEqual(result, 1)
    }))
  ...
  it.effect("someOrFail - extracts the optional value", () =>
    Effect.gen(function*() {
      const result = yield* pipe(
        ...
        Effect.someOrFail(() => ExampleError)
      )
      assert.strictEqual(result, 42)
    }))
