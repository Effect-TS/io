...
describe.concurrent("Effect", () => {
  ...
  it.effect("validateAllDiscard - returns all errors if never valid", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAllDiscard(Effect.fail),
        Effect.flip
      )
      assert.deepStrictEqual(Array.from(result), array)
    }))
  ...
  it.effect("validateAllPar - returns all errors if never valid", () =>
    Effect.gen(function*() {
      const array = Array.from({ length: 1_000 }, () => 0)
      const result = yield* pipe(
        array,
        Effect.validateAllPar(Effect.fail),
        Effect.flip
      )
      assert.deepStrictEqual(Array.from(result), array)
    }))
  ...
  it.effect("validateAllPar - accumulate errors and ignore successes", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAllPar((n) => n % 2 === 0 ? Effect.succeed(n) : Effect.fail(n)),
        Effect.flip
      )
      assert.deepStrictEqual(Array.from(result), [1, 3, 5, 7, 9])
    }))
  ...
  it.effect("validateAllPar - accumulate successes", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAllPar(Effect.succeed)
      )
      assert.deepStrictEqual(Array.from(result), array)
    }))
  ...
  it.effect("validateAllParDiscard - returns all errors if never valid", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAllParDiscard(Effect.fail),
        Effect.flip
      )
      assert.deepStrictEqual(Array.from(result), array)
    }))
  ...
  it.effect("validateFirst - runs sequentially and short circuits on first success validation", () =>
    Effect.gen(function*() {
      ...
      const result = yield* pipe(
        array,
        Effect.validateFirst((n) => pipe(counter, Ref.update((n) => n + 1), Effect.zipRight(f(n))))
      )
  ...
  describe.concurrent("", () => {
    it.effect("validateFirstPar - returns all errors if never valid", () =>
      Effect.gen(function*() {
        ...
        const result = yield* pipe(
          array,
          Effect.validateFirstPar(Effect.fail),
          Effect.flip
        )
        assert.deepStrictEqual(Array.from(result), array)
      }))
