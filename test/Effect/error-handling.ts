...
describe.concurrent("Effect", () => {
  ...
  it.effect("absolveWith right", () =>
    Effect.gen(function*($) {
      const result = yield* $(
        ...
        Effect.absolveWith(() => Either.right("absolveRight"))
      )
  ...
  it.effect("absolveWith left", () =>
    Effect.gen(function*($) {
      const result = yield* $(
        Effect.succeed("test"),
        Effect.absolveWith(() => Either.left("absolveLeft" as const)),
        Effect.exit
      )
  ...
  it.effect("orElseOptional - produces the value of this effect if it fails with some error", () =>
    Effect.gen(function*($) {
      const result = yield* $(
        Effect.fail(Option.some("fail")),
        Effect.orElseOptional(() => Effect.succeed("orElse")),
        Effect.exit
      )
  ...
  it.effect("no information is lost during composition", () =>
    Effect.gen(function*($) {
      ...
      const result = yield* $(Effect.failCause(expectedCause), Effect.sandbox, Effect.unsandbox, cause)
