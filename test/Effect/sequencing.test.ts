...
describe("Effect", () => {
  ...
  describe("", () => {
    it.effect("tapErrorCause - effectually peeks at the cause of the failure of this effect", () =>
      Effect.gen(function*($) {
        ...
        expect(Exit.isFailure(result) && Option.isSome(Cause.dieOption(result.i0))).toBeTrue()
        expect(effect).toBeTrue()
      }))
  })
  ...
  it.effect("zip/parallel - does not swallow exit causes of loser", () =>
    Effect.gen(function*($) {
      ...
      expect(HashSet.size(result)).toBeGreaterThan(0)
    }))
