  ...
  it.effect("race in daemon is executed", () =>
    Effect.gen(function*() {
      ...
      const loser2 = Effect.acquireUseRelease(
        pipe(latch2, Deferred.succeed<void>(void 0)),
        () => Effect.never(),
          ...
          Array.from(fs).reduce(
            (acc, fiber) =>
              pipe(
                acc,
                Effect.zipRight(Fiber.interrupt(fiber)),
                Effect.zipRight(pipe(ref, Ref.update((n) => n + 1)))
              ),
            Effect.unit()
          )
        )
          ...
          pipe(
            ...
            Effect.onInterrupt(() =>
              pipe(
                interrupted,
                Ref.update((_) => _ + 1),
                Effect.zipRight(pipe(done, Deferred.succeed<void>(void 0)))
              )
            ),
            Effect.fork
          )
  ...
  it.effect("raceFirst interrupts loser on success", () =>
    Effect.gen(function*() {
      ...
      const loser = Effect.acquireUseRelease(
        pipe(deferred, Deferred.succeed<void>(void 0)),
        () => Effect.never(),
  ...
  it.effect("raceFirst interrupts loser on failure", () =>
    Effect.gen(function*() {
      ...
      const winner = pipe(
        ...
        Effect.zipRight(Effect.fromEither(Either.left(new Error())))
      )
      const loser = Effect.acquireUseRelease(
        pipe(deferred, Deferred.succeed<void>(void 0)),
        () => Effect.never(),
