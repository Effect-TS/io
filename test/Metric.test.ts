...
import * as PollingMetric from "@effect/io/MetricPolling"
  ...
  describe("Frequency", () => {
    ...
    it.effect("occurences + taggedWith", () =>
      Effect.gen(function*($) {
        ...
        expect(HashMap.isEmpty(result1.occurrences)).toBeTrue()
  ...
  describe("Histogram", () => {
    ...
    it.flakyTest(
      Effect.gen(function*($) {
        ...
        assert.strictEqual(result.count, 2)
        expect(result.sum).toBeGreaterThan(0.39)
        expect(result.sum).toBeLessThanOrEqual(elapsed)
        expect(result.min).toBeGreaterThanOrEqual(0.1)
        expect(result.min).toBeLessThan(result.max)
        expect(result.max).toBeGreaterThanOrEqual(0.3)
        expect(result.max).toBeLessThan(elapsed)
      })
    )
