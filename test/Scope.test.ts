...
describe("Scope", () => {
  ...
  it.effect("runs finalizers in parallel when the scope is closed", () =>
    Effect.gen(function*($) {
      ...
      const result = yield* $(Ref.get(ref))
      expect(result.slice(0, 2).some((action) => isAcquire(action) && action.id === 1)).toBeTrue()
      expect(result.slice(0, 2).some((action) => isAcquire(action) && action.id === 2)).toBeTrue()
      expect(result.slice(2, 4).some((action) => isUse(action) && action.id === 1)).toBeTrue()
      expect(result.slice(2, 4).some((action) => isUse(action) && action.id === 2)).toBeTrue()
      expect(result.slice(4, 6).some((action) => isRelease(action) && action.id === 1)).toBeTrue()
      expect(result.slice(4, 6).some((action) => isRelease(action) && action.id === 2)).toBeTrue()
    }))
  it.effect("preserves order of nested sequential finalizers", () =>
    Effect.gen(function*($) {
      ...
      expect(action2Index).toBeLessThan(action1Index)
      expect(action4Index).toBeLessThan(action3Index)
    }))
