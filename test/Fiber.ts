...
describe.concurrent("Fiber", () => {
  ...
  it.scoped("inheritLocals works for Fiber created using map", () =>
    Effect.gen(function*($) {
      ...
      const child = yield* $(
        withLatch((release) => pipe(fiberRef, FiberRef.set(update), Effect.zipRight(release), Effect.fork))
      )
  ...
  it.effect("interruptAll interrupts fibers in parallel", () =>
    Effect.gen(function*($) {
      ...
      const fiber1 = yield* $(
        pipe(deferred1, Deferred.succeed<void>(void 0), Effect.zipRight(Effect.never()), Effect.forkDaemon)
      )
      const fiber2 = yield* $(
        pipe(
          deferred2,
