...
describe.concurrent("Layer", () => {
      ...
      const layer = Layer.scoped(ChunkTag)(
        pipe(
          Effect.acquireRelease(
            ...
            (ref) => pipe(Ref.get(ref), Effect.flatMap((chunk) => pipe(testRef, Ref.set(chunk))))
          ),
  ...
  it.scoped("sharing itself with merge", () =>
    Effect.gen(function*() {
      ...
      const layer = Layer.fromValue(Service1Tag)(() => service1)
      const env = pipe(layer, Layer.merge(layer), Layer.merge(layer), Layer.build)
      const result = yield* pipe(
        env,
        Effect.flatMap((context) => Effect.attempt(() => pipe(context, Context.get(Service1Tag))))
      )
  ...
  it.effect("orElse - uses an alternative layer", () =>
    Effect.gen(function*() {
      ...
      const env = pipe(
        layer1,
        Layer.provideToAndMerge(Layer.fail("failed!")),
        Layer.orElse(() => layer2),
        Layer.build
      )
      yield* Effect.scoped(env)
  ...
  it.effect("handles errors gracefully", () =>
    Effect.gen(function*() {
      ...
      const layer4 = Layer.scoped(ScopedTag)(
    ...
    Effect.gen(function*() {
      ...
      const env = pipe(
        ...
        Layer.provideTo(
          pipe(
            layer2,
            Layer.merge(pipe(layer1, Layer.provideTo(layer3), Layer.fresh))
          )
        ),
        Layer.build
      )
    ...
    Effect.gen(function*() {
      ...
      const env = pipe(
        ...
        Layer.provideTo(
          pipe(
            layer2,
            Layer.merge(pipe(layer1, Layer.provideTo(layer3)))
          )
        ),
        Layer.build
      )
    ...
    Effect.gen(function*() {
      ...
      const live = pipe(
        ...
        Layer.map((context) =>
          pipe(
            Context.empty(),
            Context.add(StringTag)(pipe(context, Context.get(ServiceATag)).name)
          )
        ),
  ...
  it.effect("to provides a partial environment to another layer", () =>
    Effect.gen(function*() {
      ...
      interface FooService {
        ...
        readonly get: Effect.Effect<never, never, readonly [number, string]>
      }
      ...
      const result = yield* pipe(
        Effect.serviceWithEffect(FooTag)((_) => _.get),
        Effect.provideLayer(layer)
      )
      assert.strictEqual(result[0], 10)
      assert.strictEqual(result[1], "hi")
  ...
  it.effect("passthrough passes the inputs through to the next layer", () =>
    Effect.gen(function*() {
      ...
      const live = pipe(
        Layer.fromValue(NumberTag)(() => ({ value: 1 })),
        Layer.provideTo(Layer.passthrough(layer))
      )
  ...
  it.effect("project", () =>
    Effect.gen(function*() {
      ...
      const { age } = yield* pipe(
        Effect.service(AgeTag),
        Effect.provideLayer(ageLayer)
      )
  ...
  it.effect("retry", () =>
    Effect.gen(function*() {
      ...
      const layer = pipe(
        Layer.fromEffectEnvironment(effect),
  ...
  it.effect("tap peeks at an acquired resource", () =>
    Effect.gen(function*() {
      ...
      const layer = pipe(
        Layer.fromValue(BarTag)(() => ({ bar: "bar" })),
