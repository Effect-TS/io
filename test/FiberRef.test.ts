...
import { constant, constTrue, identity } from "@effect/data/Function"
...
describe("FiberRef", () => {
  ...
  it.scoped("nothing gets inherited with a failure in zipPar", () =>
    Effect.gen(function*($) {
      ...
      const result = yield* $(FiberRef.get(fiberRef))
      expect(result.includes(initial)).toBeTrue()
    }))
  ...
  it.scoped("its value is inherited after simple race", () =>
    Effect.gen(function*($) {
      ...
      const result = yield* $(FiberRef.get(fiberRef))
      expect(new RegExp(`${update1}|${update2}`).test(result)).toBeTrue()
    }))
