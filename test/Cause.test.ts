...
describe("Cause", () => {
  ...
  it("should be compared for equality by value", () => {
    expect(Equal.equals(Cause.fail(0), Cause.fail(0))).toBeTrue()
    expect(Equal.equals(Cause.die(0), Cause.die(0))).toBeTrue()
    expect(Equal.equals(Cause.fail(0), Cause.fail(1))).toBeFalse()
    expect(Equal.equals(Cause.die(0), Cause.die(1))).toBeFalse()
  })
  ...
  it("left identity", () => {
    fc.assert(fc.property(causes, (cause) => {
      ...
      const right = cause
      expect(Equal.equals(left, right)).toBeTrue()
    }))
  })
  ...
  describe("stripSomeDefects", () => {
    it("returns `Some` with remaining causes", () => {
      ...
      expect(Equal.equals(stripped, Option.some(cause2))).toBeTrue()
    })
    ...
    it("returns `None` if there are no remaining causes", () => {
      ...
      expect(Equal.equals(stripped, Option.none())).toBeTrue()
    })
  })
