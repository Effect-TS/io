...
describe("Layer", () => {
  ...
  it.effect("finalizers", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(env))
      const result = yield* $(Ref.get(ref))
      expect(Array.from(result).slice(0, 2).find((s) => s === acquire1)).toBeDefined()
      expect(Array.from(result).slice(0, 2).find((s) => s === acquire2)).toBeDefined()
      expect(Array.from(result).slice(2, 4).find((s) => s === release1)).toBeDefined()
      expect(Array.from(result).slice(2, 4).find((s) => s === release2)).toBeDefined()
    }))
  ...
  it.effect("interruption with merge", () =>
    Effect.gen(function*($) {
      ...
      if (result.find((s) => s === acquire1) !== undefined) {
        expect(result.some((s) => s === release1)).toBeTrue()
      }
      if (result.find((s) => s === acquire2) !== undefined) {
        expect(result.some((s) => s === release2)).toBeTrue()
      }
    }))
  ...
  it.effect("interruption with multiple layers", () =>
    Effect.gen(function*($) {
      ...
      if (result.find((s) => s === acquire3) !== undefined) {
        expect(result.some((s) => s === release3)).toBeTrue()
      }
    }))
  ...
  it.effect("sharing with multiple layers with provideTo", () =>
    Effect.gen(function*($) {
      ...
      expect(result.slice(1, 3).some((s) => s === acquire2)).toBeTrue()
      expect(result.slice(1, 3).some((s) => s === acquire3)).toBeTrue()
      expect(result.slice(3, 5).some((s) => s === release3)).toBeTrue()
      expect(result.slice(3, 5).some((s) => s === release2)).toBeTrue()
